...
def main_worker(args):
    ...
    for index, items in enumerate(test_loader):
        ...
        ori_frames = [ori_frames[i].squeeze().cpu().numpy()
        ...
        for f in range(0, video_length, neighbor_stride):
            ...
            with torch.no_grad():
                ...
                binary_masks = masks[0, neighbor_ids, :, :, :].cpu().permute(0, 2, 3, 1).numpy().astype(np.uint8)
        ...
        print(f'[{index+1:3}/{len(test_loader)}] Name: {str(video_name):25} | PSNR/SSIM: {cur_psnr:.4f}/{cur_ssim:.4f}')
        eval_summary.write(f'[{index+1:3}/{len(test_loader)}] Name: {str(video_name):25} | PSNR/SSIM: {cur_psnr:.4f}/{cur_ssim:.4f}\n')
        ...
        if args.save_results:
            ...
            for i, frame in enumerate(comp_frames):
                cv2.imwrite(os.path.join(save_frame_path, str(i).zfill(5)+'.png'),
    ...
    eval_summary.write('Finish evaluation... Average Frame PSNR/SSIM/VFID: '
                       f'{avg_frame_psnr:.2f}/{avg_frame_ssim:.4f}/{fid_score:.3f}')
    eval_summary.close()
